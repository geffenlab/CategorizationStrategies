...
#[docs]
def accel_peaks(t, pos, freq=1000):

    vel, accel = velocity_smoothed(pos, freq, 0.015)
    
    # Here, I'm trying something new (JSC)- going to find all "max" points in acceleration

    p_a = accel[argrelextrema(accel, np.greater)]
    p_a_t = t[argrelextrema(accel, np.greater)]
    p_a_v = vel[argrelextrema(accel, np.greater)]    

    return p_a, p_a_t, p_a_v

#[docs]
def vel_peaks(t, pos, freq=1000):

    vel, accel = velocity_smoothed(pos, freq, 0.015)
    
    # Here, I'm trying something new (JSC)- going to find all "max" points in acceleration

    p_v = vel[argrelextrema(np.abs(vel), np.greater)]
    p_v_t = t[argrelextrema(np.abs(vel), np.greater)]

    return p_v, p_v_t

#[docs]
def movements(t, pos, go = [], resp = [], freq=1000, pos_thresh=8, t_thresh=.2, min_gap=.1, pos_thresh_onset=1.5,
    ...
    min_gap : float
        ...
        considered separate.  Movements with a gap smaller than this are 'stitched together'
    ...
    dt = np.diff(t)
    assert np.all(np.abs(dt - dt.mean()) < 1e-5), 'Values not evenly sampled'
    ...
    # peaks_accel = (accel[m + argrelextrema(accel[m:n], np.greater)] for m, n in zip(onset_samps, offset_samps))
    # peaks_accel = (v if (len(v) > 0) else np.nan for v in peaks_accel)
    # peaks_accel = np.fromiter(peaks_accel, dtype=object)

    # peaks_accel_times = (t[m + argrelextrema(accel[m:n], np.greater)] for m, n in zip(onset_samps, offset_samps))
    # peaks_accel_times = (v if (len(v) > 0) else np.nan for v in peaks_accel_times)
    # peaks_accel_times = np.fromiter(peaks_accel_times, dtype=object)

    # vel_at_peak_accel = (vel[m + argrelextrema(accel[m:n], np.greater)] for m, n in zip(onset_samps, offset_samps))
    # vel_at_peak_accel = (v if (len(v) > 0) else np.nan for v in vel_at_peak_accel)
    # vel_at_peak_accel = np.fromiter(vel_at_peak_accel, dtype=object)

    peaks_accel = accel[argrelextrema(accel, np.greater)]
    peaks_accel_times = t[argrelextrema(accel, np.greater)]
    vel_at_peak_accel = vel[argrelextrema(accel, np.greater)]
    ...
    #peaks_v = (np.max(np.abs(vel[m:n])) * np.sign( vel[m + np.max(np.abs(vel[m:n])).argmax()] ) for m, n in zip(onset_samps, offset_samps))
    #peak_vels = np.fromiter(peaks_v, dtype=float, count=onsets.size)

    peaks_v = (vel[m + argrelextrema(vel[m:n] * np.sign(vel[m]), np.greater)[0]] for m, n in zip(onset_samps, offset_samps))
    peaks_v = np.fromiter(peaks_v, dtype=object, count=onsets.size)
    peak_vels = (v[0] if (len(v) > 0) else np.nan for v in peaks_v)
    peak_vels = np.fromiter(peak_vels, dtype=object)

    peaks_v = (t[m + argrelextrema(vel[m:n] * np.sign(vel[m]), np.greater)[0]] for m, n in zip(onset_samps, offset_samps))
    peaks_v = np.fromiter(peaks_v, dtype=object, count=onsets.size)
    peak_vel_times = (v[0] if (len(v) > 0) else np.nan for v in peaks_v)
    peak_vel_times = np.fromiter(peak_vel_times, dtype=object)

    peaks_vr = (vel[m + argrelextrema(vel[m:n] * np.sign(vel[m]), np.less)[0]] for m, n in zip(onset_samps, offset_samps))
    peaks_vr = np.fromiter(peaks_vr, dtype=object, count=onsets.size)
    peak_vels_reb = (v[0] if (len(v) > 0) else np.nan for v in peaks_vr)
    peak_vels_reb = np.fromiter(peak_vels_reb, dtype=object)

    peaks_vr = (t[m + argrelextrema(vel[m:n] * np.sign(vel[m]), np.less)[0]] for m, n in zip(onset_samps, offset_samps))
    peaks_vr = np.fromiter(peaks_vr, dtype=object, count=onsets.size)
    peak_vel_times_reb = (v[0] if (len(v) > 0) else np.nan for v in peaks_vr)
    peak_vel_times_reb = np.fromiter(peak_vel_times_reb, dtype=object)
    ...
    if make_plots:
        ...
        for ax, y in zip(axes, (pos, vel)):
            ax.plot(onsets, y[onset_samps], 'go',label = 'onsets')
            ax.plot(offsets, y[offset_samps], 'bo',label = 'offsets')
            ...
            if go:
                ax.axvline(x=go, color='g', label='_nolegend_', linestyle=':')
            if resp:
                ax.axvline(x=resp, color='r',label='_nolegend_', linestyle=':')  
        
        axes[1].plot(peak_vel_times, peak_vels,'ko')
        axes[1].plot(peak_vel_times_reb, peak_vels_reb,'ko')
    ...
    return onsets, offsets, peak_amps, peak_vel_times, peak_vels, peak_vel_times_reb, peak_vels_reb, peaks_accel, peaks_accel_times, vel_at_peak_accel, axes
