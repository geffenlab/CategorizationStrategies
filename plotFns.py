...
def plot_weights(W, weight_dict=None, ax = None, colors=None, zorder=None, errorbar=None, days=None): 

    '''
    Adapted from Roy et al, 2021: Plots psytrack weights in a reasonable way.
    ...
    Returns:
    figure axis (which can be modified)
    '''
...
def plotPsych(df, plotAvg = False, ax = None, plot = 'raw'):

    '''
    Plots psychometric functions.

    Args:
    df: Dataframe generated by getPsychFit function in fitFns.py
    plotAvg: option to plot average of multiple functions
    ax: option to plot into existing plt axis
    plot: plot type: raw: just choice data, fit: choice data and psychometric fit result
...
def plotClusters(X, x_pred, nPoints, plotAvg = True):

    '''
    Plots clusters.

    Args:
    X: N x K matrix of trajectories where N = number of mice and K = 2 * number of time points. Low and High trajectories concatenated, as from generateTraceMats in clusteringFns.py
    x_pred: 1 x N vector of cluster numbers
    nPoints: number of time points, should be K/2
    plotAvg: option to plot average of clusters 

    Returns:
    figure (which can be modified)
    axis (which can be modified)    
    '''
...
def plotClustersVertThreeAx(X, X_avg, x_pred, nPoints):

    '''
    Plots clusters separately, as demonstrated in Figure 3.

    Args:
    X: N x K matrix of trajectories where N = number of mice and K = 2 * number of time points. Low and High trajectories concatenated, as from generateTraceMats in clusteringFns.py
    X_avg: N x K/2 matrix of the average of the two conditional trajectories
    x_pred: 1 x N vector of cluster numbers
    nPoints: number of time points, should be K/2
...
def plotClustersVert(X, x_pred, nPoints, plotAvg = True):
...
def plotClustersVertAvgOnly(X, x_pred, nPoints, plotAvg = True):

    '''
    Plots clusters separately, as demonstrated in Figure 3.

    Args:
    X: N x K matrix of trajectories where N = number of mice and K = number of time points, as from generateTraceMats in clusteringFns.py
    x_pred: 1 x N vector of cluster numbers
    nPoints: number of time points, should be K
...
def plotDifSizeClustersAvgOnlyOneAx(X, x_pred, nPointsVec, plotAvg = True):

    '''
    Plots clusters when they have different lengths.

    Args:
    X: N x K matrix of trajectories where N = number of mice and K = 2 * max number of time points. Low and High trajectories concatenated, as from generateTraceMats in clusteringFns.py
    x_pred: 1 x N vector of cluster numbers
    nPointsVec: vector of number of time points, each should be K/2
    plotAvg: option to plot average of clusters 
...
def plotQModel(Q_stored, H, L, binH, binL, eOT):
    ...
    '''
    Plots conditional probabilities and choices of learning model.

    Args:
    Q_stored: Vector of Q values throughout training
    H: Choice probability for High category stimuli
    L: Choice probability for Low category stimuli
    binH: Binned "High" category responses
    binL: Binned "Low" category responses
    eOT: End of training if want to only plot up until a certain trial number
...
def plotFitSimulations(sim, reps, nPoints, smoothF, smooth2):
    ...
    '''
    Plots conditional choice probabilities from simulations of learning model

    Args:
    sim: list from simulateFromParameters in reinforcementLearningFns.py file, for example
    reps: number of times simulation was run
    nPoints: number of points to subsample to for plotting
    smoothF: smoothing factor before subsampling
    smooth2: smoothing factor post-subsampling
...
def plot_and_wilcoxon(list1, list2, name1 = 'List 1', name2 = 'List 2', xl = None, yl = None):
    ...
    '''
    Plots data to be compared and computes wilcoxon signed-rank test statistics

    Args:
    list1: list of data
    list2: list of second data (same length as list1)
    name1: list1 label
    name2: list2 label
    xl: optional x axis limits
    yl: optional y axis limits
