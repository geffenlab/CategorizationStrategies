...
### Function for "all associations fit"

def simulateLearningAllAssociations(mp, stimCat = None):
    ...
    Q_stored = np.zeros((4, mp['N']), dtype = float)
    choiceProb = np.zeros((mp['N']), dtype = float)
    ...
    for t in range(mp['N'] - 1):
        ...
        wT = np.array([stim_stored[t][0], stim_stored[t][1], -1*stim_stored[t][1], -1*stim_stored[t][0]]) * Q + mp['bias']
        pH = 1/(1 + np.exp(-(5 * np.exp(-mp['theta'])) * np.sum(wT)))
        ...
        Q[int(choice[t])] += mp['alpha'] * delta

        Q[int(choice[t])+2] += np.exp(-mp['beta']) * mp['alpha'] * delta
...
def fitLearningAllAssociations(guesses, mp, choice, stim_cat):

    #print(guesses) # Initial Q [L/CW, H/CCW, H/CW, L/CCW]
    ...
    if guesses is not None:
        if len(guesses) == 4:
            ...
            mp['alpha'] = alpha_guess
            mp['beta'] = beta_guess
            mp['init_Q'] = [q_init_guess, -1*q_init_guess, q_init_guess, -1*q_init_guess]
        elif len(guesses) == 3:
            bias_guess, alpha_guess, beta_guess = guesses
            mp['bias'] = bias_guess
            mp['alpha'] = alpha_guess
            mp['beta'] = beta_guess
        elif len(guesses) == 2:
    ...
    if np.isnan(mp['bias']) or np.isnan(mp['alpha']) or np.isnan(mp['beta'] or np.isnan(mp['init_Q'])): # check inputs
        ...
        for t in range(mp['N']):
            ...
            if mp['beta'] < 100:
                Q[int(choice[t])+2] += mp['alpha'] * np.exp(-mp['beta']) * delta
...
def simulateFromFitAllAssociations(ID, mfN, R = 1):

    dataBase = os.path.abspath(os.path.join(curPath,"data/Trajectories/with_bias_learning"))

    f = loadModelFitAA(mfN)
    loaded_dict = mf.loadSavedFits(ID, dataBase, ending = '_trainingDataBias')
    ...
    if mfN > 3:
        qT = f.loc[f['id'] == ID,'init_Q'].iloc[0]
        model_params['init_Q'] = [qT, -1 * qT, qT, -1 * qT]
    ...
    for r in range(0,R):
        ...
        stim_stored, choice, reward, Q_stored, choiceProb =  simulateLearningAllAssociations(model_params, stimCat = cat - 1)
        sim_h, sim_l, hx, lx, h_prob, l_prob, eOT = generateLearningCurves(stim_stored, choice, choiceProb, learnedCutOff = learnedCutOff) #[0:2]
...
def loadModelFitAA(num_params, top = 1, folder = 'model_fit_data'):

    keyword = 'fits_AA_' + str(num_params) + 'param'
